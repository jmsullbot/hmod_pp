...
def make_delta_pk_callable(dndM_extra_fn, cosmo,
    ...
    The 2-halo contribution from the extra halos is not included here, as
    it would require specifying their large-scale bias.
    ...
    k_lo, k_hi : float
        k grid range [h/Mpc].
    n_k : int
        Number of k points in precomputed grid.
    z_lo, z_max : float
        Redshift grid range.
    n_z : int
        Number of z points in precomputed grid.
    M_lo, M_hi : float
    ...
    delta_pk_grid : 2-D array, shape (n_k, n_z)
        Raw precomputed grid values [(Mpc/h)^3].
    ...
    for j, z in enumerate(z_grid):
        rho_m = cosmo.rho_m(z)
        dndM_extra = dndM_extra_fn(M_arr, z)   # shape (n_M,)
    ...
    # 2-D linear interpolator over (log k, z)
